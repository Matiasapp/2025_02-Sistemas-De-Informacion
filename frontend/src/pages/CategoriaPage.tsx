...
export function CategoriaPage() {
    ...
    const fetchData = async () => {
      try {
        const [productsRes, colorsRes] = await Promise.all([
          fetch(`${backendUrl}/categoria/${encodedCategory}`),
        ...
        const uniqueBrands = Array.from(
          new Set(
            productsData.map((p: Product) => p.brand_name).filter(Boolean)
          )
        ) as string[];
  ...
  const filteredAndSortedProducts = productos
    .filter((p) => {
      ...
      const matchesPrice = p.min_price <= priceRange[1];
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 bg-gray-50 p-4 rounded-lg">
        ...
        {/* Segunda fila: Rango de precios y Ordenar */}
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Rango de precios - Un tercio */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Precio máximo: $
              {Math.floor(priceRange[1]).toLocaleString("es-CL")}
            </label>
            <input
              type="range"
              min={minPrice}
          ...
          {/* Botón limpiar filtros - Un tercio */}
      ...
      <p className="text-gray-600 mb-4">
        Mostrando {filteredAndSortedProducts.length} de {productos.length}{" "}
        productos
      </p>
